 ...
 *	@retval NULL		Failed.
 */
void *	lagopus_malloc_on_numanode(size_t sz, unsigned int node);
...
/**
 * Free memory allocated by the \b lagopus_malloc_on_numanode().
 *
 *	@param[in]	p	A pointer to memory.
 */
void	lagopus_free_on_numanode(void *p);
